...
async function run() {
  ...
  if (res.ok) {
    ...
    console.log("ledgerGetInfo failed:", res.error);
  }
}
...
async function run() {
  const result = await sdk.ledger.getMetrics();
...
```typescript
import { SDKCore } from "@formance/formance-sdk/core.js";
import { ledgerGetMetrics } from "@formance/formance-sdk/funcs/ledgerGetMetrics.js";
...
async function run() {
  const res = await ledgerGetMetrics(sdk);
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("ledgerGetMetrics failed:", res.error);
  }
}
