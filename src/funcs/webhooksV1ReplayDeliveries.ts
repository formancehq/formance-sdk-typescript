 ...
 */
export function webhooksV1ReplayDeliveries(
  client: SDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ReplayDeliveriesRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.ReplayDeliveriesRequest, {
    explode: true,
  });

  const path = pathToFunc("/api/webhooks/deliveries/replay")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ReplayDeliveriesResponse$inboundSchema, {
      key: "ReplayDeliveriesResponse",
    }),
