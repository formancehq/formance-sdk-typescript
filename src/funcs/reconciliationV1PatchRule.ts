 ...
 */
export function reconciliationV1PatchRule(
  client: SDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.PatchRuleRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.RulePatchRequest, { explode: true });
  ...
  const path = pathToFunc("/api/reconciliation/rules/{ruleID}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.PatchRuleResponse$inboundSchema, {
      key: "RuleResponse",
    }),
