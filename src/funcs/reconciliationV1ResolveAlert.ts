 ...
 */
export function reconciliationV1ResolveAlert(
  client: SDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ResolveAlertRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.ResolveAlertRequest, {
    explode: true,
  });
  ...
  const path = pathToFunc("/api/reconciliation/alerts/{alertID}/resolve")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ResolveAlertResponse$inboundSchema, {
      key: "AlertResponse",
    }),
