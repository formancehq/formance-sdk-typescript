 ...
 */
export function paymentsV1UpdatePoolQuery(
  client: SDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdatePoolQueryRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.UpdatePoolQueryRequest, {
    explode: true,
  });
  ...
  const path = pathToFunc("/api/payments/pools/{poolId}/query")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.nil(204, operations.UpdatePoolQueryResponse$inboundSchema),
