 ...
 */
export function reconciliationV1AcceptAlert(
  client: SDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.AcceptAlertRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.AcceptAlertRequest, {
    explode: true,
  });
  ...
  const path = pathToFunc("/api/reconciliation/alerts/{alertID}/accept")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.AcceptAlertResponse$inboundSchema, {
      key: "AlertResponse",
    }),
