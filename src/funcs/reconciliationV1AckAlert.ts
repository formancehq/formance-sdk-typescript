 ...
 */
export function reconciliationV1AckAlert(
  client: SDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.AckAlertRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.AckAlertRequest, { explode: true });
  ...
  const path = pathToFunc("/api/reconciliation/alerts/{alertID}/ack")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.AckAlertResponse$inboundSchema, {
      key: "AlertResponse",
    }),
