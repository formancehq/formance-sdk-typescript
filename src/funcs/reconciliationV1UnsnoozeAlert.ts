 ...
 */
export function reconciliationV1UnsnoozeAlert(
  client: SDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UnsnoozeAlertRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.UnsnoozeAlertRequest, {
    explode: true,
  });
  ...
  const path = pathToFunc("/api/reconciliation/alerts/{alertID}/unsnooze")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.UnsnoozeAlertResponse$inboundSchema, {
      key: "AlertResponse",
    }),
