...
export function readTriggerRequestFromJSON(
  jsonString: string,
): SafeParseResult<ReadTriggerRequest, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => ReadTriggerRequest$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'ReadTriggerRequest' from JSON`,
  );
}
