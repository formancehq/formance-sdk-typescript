...
export function transactionDataFromJSON(
  jsonString: string,
): SafeParseResult<TransactionData, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => TransactionData$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'TransactionData' from JSON`,
  );
}
